...
## v0.5.244
- **Account switcher hidden for single-account users + page descriptions shortened.** The "ACCOUNT: Saruba" row (introduced in v0.5.158) rendered on all three account tabs even for users who own only one subscription — wasting vertical space and creating a confusing "why is my account name in a row with a label" affordance with no switching purpose. Fix: `renderSwitcher()` in all three pages now hides the row entirely when `allSubs.length <= 1`. It only appears for coaches who own 2+ subscriptions. Page-sub descriptions on Businesses and Users shortened to single lines ("Manage businesses in this account." / "Team members across all businesses in this account.") — previously two lines on phone, adding visual weight. **Also created `supabase/v0.5.244-delta.sql`**: a `users` table RLS SELECT policy that lets active team members see co-members' emails. The `memberEmailMap` query in v0.5.243 was already written correctly, but the current `users` RLS only exposes your own row to the client — so Andrew's email lookup returned nothing. Once this SQL is run, Andrew's card will show his email below "Andrew". **Requires SQL** — run `supabase/v0.5.244-delta.sql` in the Supabase SQL Editor.
...
## v0.5.240
- **Bugfix: login loop redirecting to setup wizard instead of login page.** `index.html`'s top-level auth/profile checks were missing `return` statements after their redirects. If any Supabase query failed (expired auth token, transient network error, RLS issue), execution fell through into `loadAll()`. The subscriptions query also failed silently (`allSubs = subs || []` with `subs` being undefined), `allSubs` became `[]`, and the `if (!allSubs.length)` guard fired — sending the user to `setup.html` (the "add a new business" wizard) instead of the correct `inactive.html` or `login.html`. The `window.location.href = 'setup.html'` was the last href assignment, so it won the navigation race. **Fix:** Moved the entire auth gate + profile check + signout-listener + `await refresh()` + post-boot rendering into a proper `async boot()` function with `return` after each redirect — matching the pattern already used in `account-users.html`. Also added explicit error handling on the subscriptions query: if it returns an error (vs. empty), the handler now redirects to `login.html` rather than falling through to setup.html. No SQL.
...
## v0.5.228
- **Architecture correction + diagnostic SQL.** Documented the correct coach/client architecture in `CLAUDE.md` under a new `## Coach + Client Architecture` section: Cath (cath@coach4u.com.au) owns all three subscriptions (Coach4U / SARUBA / IAS) and SARUBA/IAS clients have no separate logins — all three subscriptions appear in the account switcher via `owner_user_id`, not via team membership. Created `supabase/v0.5.228-diagnostic.sql` with queries to find why SARUBA/IAS may be missing from the switcher (wrong owner_user_id or deleted), with fix SQL for both cases. Rewrote `supabase/v0.5.226-coach-setup.sql` to require identifying the correct subscription ID before running (to avoid accidentally setting SARUBA/IAS to `subscription_type = 'coach'`). Clarified that v0.5.227-delta.sql Part 2 (inserting Cath as team member in non-owned orgs) should NOT be run — the correct fix is ensuring SARUBA/IAS subscriptions have `owner_user_id = Cath's user ID`. **No code changes — diagnostic and documentation only.**
